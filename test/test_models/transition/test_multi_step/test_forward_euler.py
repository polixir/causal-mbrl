...
class TestForwardEulerTransition(TestCase):
    def setUp(self) -> None:
        ...
        self.repeat_times = 3
        self.device = "cuda"
        self.deterministic_one_step = PlainEnsembleGaussianTransition(obs_size=self.obs_size,
                                                                      ...
                                                                      deterministic=True)
        self.deterministic_transition = ForwardEulerTransition(one_step_transition=self.deterministic_one_step,
                                                               repeat_times=self.repeat_times)
        self.gaussian_one_step = PlainEnsembleGaussianTransition(obs_size=self.obs_size,
                                                                 action_size=self.action_size,
                                                                 device=self.device,
                                                                 num_layers=self.num_layers,
                                                                 ensemble_num=self.ensemble_num,
                                                                 hid_size=self.hid_size,
                                                                 deterministic=False)
        self.gaussian_transition = ForwardEulerTransition(one_step_transition=self.gaussian_one_step,
                                                          repeat_times=self.repeat_times)
