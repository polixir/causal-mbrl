...
class TestBasicEnsembleGaussianMLP(TestCase):
    def setUp(self) -> None:
        ...
        self.batch_size = 128
        self.device = "cuda"
        self.deterministic_transition = PlainEnsembleGaussianTransition(obs_size=self.obs_size,
                                                                        ...
                                                                        deterministic=True)
        self.gaussian_transition = PlainEnsembleGaussianTransition(obs_size=self.obs_size,
                                                                   action_size=self.action_size,
                                                                   device=self.device,
                                                                   num_layers=self.num_layers,
                                                                   ensemble_num=self.ensemble_num,
    ...
    def test_load(self):
        ...
        new_gaussian_transition = PlainEnsembleGaussianTransition(obs_size=self.obs_size,
                                                                  action_size=self.action_size,
                                                                  device=self.device,
                                                                  num_layers=self.num_layers,
                                                                  ensemble_num=self.ensemble_num,
